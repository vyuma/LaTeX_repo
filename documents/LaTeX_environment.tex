...
VSCodeでのビルド時にエラーが起きた場合、VScode はビルドを再試行する。この際右下に
Recipe terminated with error. Retry building the project.\index{Recipe terminated with error. Retry building the project.}
というポップアップが表示される。
2回目のビルドでもエラーが起きた場合、
Recipe terminated with error.\index{Recipe terminated with error}
というポップアップが表示される。
まずはOpen compiler logボタンをクリックしてエラーメッセージを表示する。

\subsection{原因の切り分け}
エラーメッセージが出ていることを確認したら、エラーが出ていそうな部分をコメントアウトしてからビルドするなどして、
エラーが出ていない部分を確定させる。
こうすることで、見当違いの場所に修正を施して複雑化するなどの事態を避けることができる。
ここまでは大丈夫という確認のためにも、少し書き上げたらその都度コンパイルしてみる。
...
エラーメッセージはエラーが起こった個所を示してくれるが、そこが実際に修正を施すべき箇所であるとは限らない。

\subsection{解決方法の探し方}
まずはエラーメッセージをよく読もう。英語だからと読み飛ばしてはいけない。
エラーメッセージには原因が書いてある。
原因からすぐに解決方法が分かる場合は、その方法を実行しよう。
そうでない場合はエラーメッセージをそのままコピーして検索すると、大体は偉大な先人が残してくれた解決策が見つかる。
% \cite{TeXwiki-home}や\cite{StackOverflow}などの情報が助けになるだろう。

\subsection{よくあるエラー}
\subsubsection{Undefined control sequence.}
コントロールシークエンス、つまりコマンドが定義されていないというエラーである。
この場合はコマンドのスペルミス、あるいはコマンドの後のスペースの入れ忘れを疑うべきである。

\subsection{align環境の空行}
align環境は空行を入れるとエラーが起きる。
\subsection{かっこの閉じ忘れ}
このエラーは、エラーメッセージからは原因が推測しにくいという点で少し難しい。
