...
\footnote{日本では「ラテフ」と読むことが多い。たまに「レイテック」「ラテック」派がいる。\LaTeX の読み方については、\cite{文書システムLaTeX}\cite{LaTeXの読み方}を参照すること。ただし\TeX は確実に「テフ」と読むこと。これは「テックス」ではない\TeX のXは英語のXではなく\(\chi\)の発音に由来する。これは\cite{TeXの発音について}を参照。}\footnote{\LaTeX はLeslie Lamport\index{Leslie Lamport}によって開発された組版システムのことである。\TeX よりもより簡単に記述できるようになっている。}
...
統合開発環境である。要は高性能なエディター\index{エディター}であり、後述する拡張機能の存在によって\LaTeX との親和性も高い。\footnote{テキストエディタはWindows標準搭載のメモ帳などが有名である。またTeX専用のテキストエディタとしてはTeXliveにあるTeXworks等が有名である。この資料では今後のトレンドを考えてVSCodeへの導入を推奨する。}
VS Codeは、
\begin{itemize}
  \item 無料で頒布されている
  \item 動作が軽い
...
赤い文字に怒られたように感じるかもしれない。
しかしそれは勘違いである。
エラーメッセージは修正するヒントを与えてくれているのである。

エラーが起きた時の典型的な解決手順を順番に解説する。
\subsection{エラーメッセージの表示方法}
VSCodeでのコンパイル時にエラーが起きた場合、
右下にRecipe terminated with error.
\index{Recipe terminated with error}
というポップアップが表示される。
...
% というポップアップも同時に表示されるが、これは一度失敗した時、再度ビルドを試行している

\begin{align}
  
\end{align}
\subsection{原因の切り分け}
エラーメッセージが出ていることを確認したら、エラーが出なくなるまでファイルの内容を戻す
ここまでは大丈夫という確認のためにも、少し書き上げたらその都度コンパイルしてみよう。
\subsection{エラーメッセージの読み方}
原因となる部分が分かったら、エラーメッセージを読んでみよう。
エラーメッセージの中に
LaTeX Error
と書かれた部分があるはずだ。
そのうち一番最初に出てくるものの解決にとりかかろう。
\subsection{解決方法の探し方}
まずはエラーメッセージを読もう。英語だからと読み飛ばしてはいけない。
エラーメッセージには原因が書いてある。
エラーメッセージを読んでも解決方法が分からない場合は、そのエラーメッセージをググろう。
そのままコピーして検索すると大体は偉大な先人が残してくれた解決策が見つかる。

\subsection{よくあるエラー}
\subsubsection{Undefined control sequence.}
コントロールシークエンス、つまりコマンドが定義されていないというエラーである。この場合はコマンドのスペルミスを疑うべきである。

\subsubsection{Missing \$ inserted.}
数式モードの中で数式モードでないものを書いているときに出るエラーである。
\subsection{align環境の空行}

\subsubsection{それでも解決しなかったら}
\LaTeXの基本的な機能を使っているうちは大抵は上の方法で解決する。
もしインターネット上の情報を見ても解決しない場合は、マニュアルを参照しよう。
\subsection{\LaTeX のマニュアル}
texdoc はTeXLiveに含まれるドキュメントを検索できるコマンドである
ターミナルで調べたい単語
例えばtexdoc
