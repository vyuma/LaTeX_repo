...
\footnote{日本では「ラテフ」と読むことが多い。たまに「レイテック」「ラテック」派がいる。また\LaTeX に関しては「テックス」とよんでもよいと言われている。ただし\TeX は確実に「テフ」と読むこと。これは「テックス」ではない\TeX のXは英語のXではなく\(\chi\)のように発音する為である。これは参考文献\cite{TeXの発音について}を参照。}
...
\begin{itemize}
  \item 無料で頒布されている
  \item 動作が軽い
  \item クロスプラットフォーム\index{クロスプラットフォーム}である（さまざまなOSに対応している）
  \item 最新トレンド全部入り
...
ソースコード\ref{code:helloWorld}の内容の.texファイルを作成し、VScode上でビルドする。このコードはモダン\LaTeX である\LuaLaTeX でコンパイルされる。
% jvlistingでコードを貼り付ける
...
\begin{description}
  ...
  \end{description}
   \item[方法２]latexworkshopの設定を用いる。コマンドパネルを開いて、 ``latex-workshop.texcount''のコマンドを用いる。
   \begin{description}
    \item[欠点]
        \begin{itemize}
          \item ステータスバーに常に表示されていないこと。
...
cloudlatexとは、株式会社アカリクが運営している\LaTeX のコンパイルをリモート環境で行うサービスのことである。このcloudlatexを用いるとローカル環境に\LaTeX がインストールされていなくとも\LaTeX 文章を書くことができる。
...
setting.jsonの\verb|{}|の中に次を入れることでlatexworkshopが対応していないパッケージの数式のレタリングをすることができる。
