...
TeX Live Managerの開き方はスタートメニューからTeX live Managerを検索すると開くことができる。

TeX live Managerを開くと現在TeX liveのパッケージのうちでローカルにインストール済みのものが表示される。
状態を「未インストール」に変更すると、
自分がローカルに入れていないパッケージが表示される。
検索欄で今自分が必要なパッケージを検索することでインストールすることができる。

\LaTeX 導入の段階で「高度な設定」を用いてインストールした場合には
必要最小限のパッケージしかインストールされていないのでここからインストール必要がある。
例えばtexworksなどが必要であればここからインストールすることができる。
