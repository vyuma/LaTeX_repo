...
それ以外のことを知りたい場合は、付録の参考文献やLaTeX workshopのreadme\cite{LaTeXworkshop_README}を
読むとよい。
...
\end{lstlisting}
\section{Table Generator\index{Table Generator}}
\LaTeX での表を書く際に非常に有用なサイトである。このサイトではwordとほとんど同等の環境のGUIを用いることで\LaTeX 記法での表に変換してくれるサイトである。
...
このように、\verb|"key"|には、キーバインドを打ち込み、\verb|"command"|には拡張機能または個のVS Codeのシステム自体に設定されている機能を記述する。
